...
def _apply_font(rng, fmt):
    """Apply font formatting using cross-platform xlwings API."""
    if 'bold' in fmt:
    ...
    if 'fontColor' in fmt:
        rng.font.color = _parse_hex_to_tuple(fmt['fontColor'])


def _apply_background(rng, fmt):
    """Apply background color using cross-platform xlwings API."""
    if 'backgroundColor' in fmt:
        rng.color = _parse_hex_to_tuple(fmt['backgroundColor'])


def _apply_number_format(rng, fmt):
    """Apply number format using cross-platform xlwings API."""
    if 'numberFormat' in fmt:
        rng.number_format = fmt['numberFormat']


def _apply_underline(rng, fmt):
    """Apply underline (requires platform-specific API)."""
    if 'underline' not in fmt:
        return
    val = 2 if fmt['underline'] else -4142  # xlUnderlineStyleSingle / xlNone
    try:
        if IS_WINDOWS:
...
def _apply_alignment(rng, fmt):
    """Apply text alignment (requires platform-specific API)."""
    h = fmt.get('textAlign')
...
def _apply_borders(rng, borders_config):
    """Apply border formatting (requires platform-specific API)."""
    for position, config in borders_config.items():
        ...
        # Determine which border positions to set
        if position == 'outside':
...
def _parse_hex_to_tuple(hex_color):
    """Convert #RRGGBB to (r, g, b) tuple for xlwings."""
    if hex_color.startswith('#'):
        hex_color = hex_color[1:]
    return (int(hex_color[0:2], 16), int(hex_color[2:4], 16), int(hex_color[4:6], 16))


def format_cells(workbook, cell_range, fmt, sheet=None):
    app, err = get_app()
    if err:
        return {"error": err}
    ...
    try:
        # Cross-platform formatting (xlwings native)
        _apply_font(rng, fmt)
        _apply_background(rng, fmt)
        _apply_number_format(rng, fmt)

        # Platform-specific formatting
        _apply_underline(rng, fmt)
        _apply_alignment(rng, fmt)
...
def main():
    ...
    result = format_cells(args.workbook, args.range, fmt, args.sheet)
    output_json(result)
