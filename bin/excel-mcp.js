#!/usr/bin/env node
process.env.EXCEL_MCP_BUNDLED = 'true';
...
// src/tools.js
var schemas = {
  executeVBA: external_exports.object({
    vbaCode: external_exports.string().describe("The VBA code to execute"),
    workbookName: external_exports.string().describe("Excel workbook name (required)"),
    moduleName: external_exports.string().optional().describe("Optional module name (default: TempModule)"),
    procedureName: external_exports.string().optional().describe("Optional procedure name (default: Main)"),
    sheetName: external_exports.string().optional().describe("Optional sheet name to navigate to before executing VBA")
  }),
  setActiveWorkbook: external_exports.object({
    workbookName: external_exports.string().describe("Name of the workbook to activate")
  }),
  getAllSheetNames: external_exports.object({
    workbookName: external_exports.string().describe("Excel workbook name (required)")
  }),
  navigateToSheet: external_exports.object({
    workbookName: external_exports.string().describe("Excel workbook name (required)"),
    sheetName: external_exports.string().describe("The name of the sheet to navigate to")
  }),
  editCells: external_exports.object({
    workbookName: external_exports.string().describe("Excel workbook name (required)"),
    range: external_exports.string().describe('Cell or range to edit (e.g., "A1" or "A1:B5")'),
    ...
    sheetName: external_exports.string().optional().describe("Optional sheet name to navigate to before editing cells")
  }),
  readSheetData: external_exports.object({
    workbookName: external_exports.string().describe("Excel workbook name (required)"),
    startRow: external_exports.number().optional().describe("Starting row number (default: 1)"),
    endRow: external_exports.number().optional().describe("Ending row number (default: 100)"),
    sheetName: external_exports.string().optional().describe("Optional sheet name. If not specified, uses the active sheet")
  }),
  getCellFormats: external_exports.object({
    workbookName: external_exports.string().describe("Excel workbook name (required)"),
    startRow: external_exports.number().optional().describe("Starting row number (default: 1)"),
    startCol: external_exports.number().optional().describe("Starting column number (default: 1)"),
    endRow: external_exports.number().optional().describe("Ending row number (default: 20, max: 35 rows from start)"),
    endCol: external_exports.number().optional().describe("Ending column number (default: 15, max: 15 columns from start)"),
    sheetName: external_exports.string().optional().describe("Optional sheet name. If not specified, uses the active sheet")
  })
};
var toolDefinitions = [
...
var ToolHandlers = class {
  ...
  async executeVBA(args) {
    ...
    if (sheetName)
      pythonArgs.push("--sheet", sheetName);
  ...
  async editCells(args) {
    ...
    if (validated.sheetName)
      pythonArgs.push("--sheet", validated.sheetName);
    return this.executePython("edit_cells.py", pythonArgs);
  }
  async readSheetData(args) {
    const validated = schemas.readSheetData.parse(args);
    const pythonArgs = [];
    if (validated.startRow)
      pythonArgs.push("--start-row", String(validated.startRow));
    if (validated.endRow)
      pythonArgs.push("--end-row", String(validated.endRow));
    pythonArgs.push("--filename", validated.workbookName);
    if (validated.sheetName)
      pythonArgs.push("--sheet", validated.sheetName);
    return this.executePython("read_sheet_data.py", pythonArgs);
  }
  async getCellFormats(args) {
    ...
    if (validated.startRow)
      pythonArgs.push("--start-row", String(validated.startRow));
    if (validated.startCol)
      pythonArgs.push("--start-col", String(validated.startCol));
    if (validated.endRow)
      pythonArgs.push("--end-row", String(validated.endRow));
    if (validated.endCol)
      pythonArgs.push("--end-col", String(validated.endCol));
    pythonArgs.push("--filename", validated.workbookName);
    if (validated.sheetName)
      pythonArgs.push("--sheet", validated.sheetName);
...
var getScriptsPath = () => {
  return process.env.EXCEL_MCP_BUNDLED ? join2(__dirname, "scripts") : join2(__dirname, "..", "scripts");
};
var ExcelMCPServer = class {
  constructor() {
